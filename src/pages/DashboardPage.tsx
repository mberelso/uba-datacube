...
import {
  ...
  BookOpen, Translate, ChartBar, ArrowRight, Warning, Wind, SunHorizon,
...
function SolarMapTeaser() {
  const [summary, setSummary] = useState<WindSummary | null>(null)

  useEffect(() => {
    fetch('/pv_summary.json').then(r => r.json()).then(setSummary).catch(() => {})
  }, [])

  const chartData = summary
    ? summary.years.map((y, i) => ({ year: String(y), value: summary.cumGw[i] })).filter(d => Number(d.year) >= 2000)
    : []
  const latest = summary ? summary.cumCount[summary.cumCount.length - 1] : null
  const latestGw = summary ? summary.cumGw[summary.cumGw.length - 1] : null
  ...
  return (
    <motion.div
      ...
      className="mb-12"
    >
      <Link to="/solar" className="block no-underline group">
        <div
          className="relative rounded-[1.5rem] overflow-hidden border border-slate-200/60 shadow-[0_4px_24px_-8px_rgba(0,0,0,0.06)]"
          style={{ background: 'linear-gradient(110deg, #1B2B3A 0%, #7c4a1d 60%, #ea580c 135%)' }}
        >
          <div className="grid grid-cols-1 md:grid-cols-[1fr_280px] gap-2 items-center">
            <div className="p-7 md:p-8">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.12)' }}>
                  <SunHorizon size={17} weight="duotone" color="#fbbf24" />
                </div>
                <span className="text-[10px] font-bold tracking-[0.12em] uppercase" style={{ color: '#fbbf24' }}>
                  Interaktive Karte
                </span>
              </div>
              <h3 className="text-[20px] font-extrabold text-white tracking-tight leading-snug m-0 mb-2">
                Der Solar-Ausbau seit 2000 — Landkreis für Landkreis
              </h3>
              <p className="text-[13px] leading-relaxed m-0 mb-4" style={{ color: 'rgba(255,255,255,0.7)' }}>
                {latest != null
                  ? `${(latest / 1e6).toLocaleString('de-DE', { maximumFractionDigits: 2 })} Mio. Photovoltaik-Anlagen mit ${latestGw?.toLocaleString('de-DE', { maximumFractionDigits: 1 })} GW Leistung — sieh zu, wie sich die Energiewende über Deutschland ausbreitet.`
                  : 'Alle Photovoltaik-Anlagen aus dem Marktstammdatenregister, animiert auf der Deutschlandkarte.'}
              </p>
              <span className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-white group-hover:gap-2.5 transition-all">
                Karte erkunden <ArrowRight size={14} weight="bold" />
              </span>
