...
import { Plant, Play, Pause, Leaf, CloudSun, TrendUp } from '@phosphor-icons/react'
...
export default function VegetationPage() {
  ...
  useEffect(() => {
    fetch(`${DATA_DIR}/meta.json`)
      .then(r => r.json())
      .then((m: Meta) => { setMeta(m); setSel(m.periods.length - 1) })
      .catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    timer.current = window.setInterval(() => {
      setSel(prev => {
        if (prev >= meta.periods.length - 1) { setPlaying(false); return prev }
        return prev + 1
      })
    }, 1200)
  ...
  // Saisonaler Verlauf: pro Monat Mittel/Min/Max über alle Jahre + gewähltes Jahr
  const seasonal = useMemo(() => {
    ...
    return MONTHS_DE.map((label, i) => {
      ...
      return {
        label, lo, mean,
        band: lo != null && hi != null ? +(hi - lo).toFixed(3) : null,
        cur: curByMonth[m] ?? null,
      }
    })
  }, [meta, sel])

  // Langzeit-Zeitreihe für Recharts + Hervorhebungs-Bereich des gewählten Jahres
  const timeline = useMemo(() => meta?.timeline.map(p => ({
    m: p.date.slice(0, 7), label: p.date.slice(0, 7), ndvi: p.ndvi,
  })) ?? [], [meta])
  ...
  if (!meta) {
    return (
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '60px 20px', color: '#64748b' }}>
  ...
  return (
    <>
      <SEO
        title="Vegetationsgesundheit Ebersberg"
        description="Sommer-Vegetation (NDVI) im Landkreis Ebersberg aus Sentinel-2-Satellitendaten — wolkenbereinigt, Jahr für Jahr vergleichbar."
        path="/vegetation"
      />
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '28px 20px 56px' }}>
        ...
        <div style={{ fontSize: 12.5, color: '#94a3b8', marginBottom: 8 }}>
          Dashboard · Vegetationsgesundheit
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          <span style={{
            background: '#4A6741', color: '#fff', borderRadius: 5,
            padding: '3px 9px', fontSize: 10, fontWeight: 700, letterSpacing: 0.6,
          }}>SATELLIT</span>
          <span style={{ fontSize: 12, color: '#64748b' }}>{meta.source}</span>
        </div>
        <h1 style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.5px', margin: '0 0 8px', color: '#0f172a' }}>
          ...
          Vegetationsgesundheit · {meta.region}
        </h1>
        <p style={{ color: '#475569', fontSize: 15, maxWidth: 700, margin: 0, lineHeight: 1.55 }}>
          {meta.indicator} aus Sentinel-2, wolkenbereinigt und auf die Landkreisfläche
          zugeschnitten. Schieberegler ziehen oder ▶ drücken, um die Sommer Jahr für Jahr
          zu vergleichen — Karte und Auswertung wechseln gemeinsam.
        </p>

        {/* ── Scrubber ─────────────────────────────────────────────────── */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, margin: '24px 0 18px' }}>
          <button
            onClick={() => setPlaying(v => !v)}
            aria-label={playing ? 'Pause' : 'Abspielen'}
            style={{
              flex: 'none', width: 44, height: 44, borderRadius: '50%', border: 'none',
              background: '#1B2B3A', color: '#fff', cursor: 'pointer',
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
              <span style={{ fontSize: 22, fontWeight: 700, color: '#1B2B3A', letterSpacing: '-0.5px' }}>
                Sommer {p.year}
              </span>
              <span style={{ fontSize: 13, color: '#64748b' }}>Median-NDVI {fmt(p.median)}</span>
            </div>
            <input
              type="range" min={0} max={meta.periods.length - 1} step={1} value={sel}
              ...
              {meta.periods.map((per, i) => (
                <button key={per.year} onClick={() => { setPlaying(false); setSel(i) }}
                  style={{
                    ...
                    fontSize: 11.5, fontWeight: i === sel ? 700 : 500,
                    color: i === sel ? '#1B2B3A' : '#94a3b8',
                  }}>
                  {per.year}
                </button>
              ))}
        ...
        {/* ── Grid: Karte + Auswertung ────────────────────────────────── */}
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.05fr) minmax(0,0.95fr)', gap: 18 }}
          className="veg-grid">
          {/* Karte */}
          <Card title="Sommer-Komposit">
            <div style={{
              position: 'relative', borderRadius: 12, overflow: 'hidden',
              background: '#f1f5f9', aspectRatio: '814 / 1000',
            }}>
              <AnimatePresence mode="popLayout">
                <motion.img
                  key={p.year}
                  src={`${DATA_DIR}/${p.img}`}
                  alt={`NDVI Sommer ${p.year}`}
              ...
              <span style={{
                position: 'absolute', left: 10, top: 10, background: 'rgba(27,43,58,0.9)',
                color: '#fff', fontSize: 13, fontWeight: 600, padding: '5px 11px', borderRadius: 8,
              }}>Sommer {p.year}</span>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 13, fontSize: 11, color: '#64748b' }}>
              <span>gestresst</span>
              <div style={{
                height: 9, flex: 1, borderRadius: 3,
          ...
          <Card title={`Auswertung ${p.year}`}>
            ...
            <div style={{ fontSize: 13.5, color: '#475569', lineHeight: 1.6, marginBottom: 18 }}>
              ...
              <b style={{ color: diff >= 0 ? '#16a34a' : '#dc2626' }}>
                {diff >= 0 ? '▲' : '▼'} {fmt(Math.abs(diffPct), 1)} %
              </b>{' '}
            ...
            <ResponsiveContainer width="100%" height={210}>
              <ComposedChart data={seasonal} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
                ...
                {/* Min–Max-Band: transparente Basis (lo) + sichtbares Band (hi-lo) gestapelt */}
              ...
              <ResponsiveContainer width="100%" height={230}>
                <LineChart data={timeline} margin={{ top: 6, right: 12, left: -18, bottom: 0 }}>
                  ...
                  <XAxis dataKey="m" tick={{ fontSize: 10, fill: '#94a3b8' }}
                    ticks={timeline.filter(t => t.m.endsWith('-01')).map(t => t.m)}
                    tickFormatter={m => m.slice(0, 4)} />
                  <YAxis domain={[0.2, 1]} tick={{ fontSize: 10, fill: '#94a3b8' }} />
                  <Tooltip content={<GlassTooltip />} />
                  {highlight && (
                    <ReferenceArea x1={highlight.x1} x2={highlight.x2} fill="#16a34a" fillOpacity={0.1} />
                  )}
        ...
        <p style={{ color: '#64748b', fontSize: 12.5, marginTop: 22, lineHeight: 1.65, borderTop: '1px solid #e2e8f0', paddingTop: 15 }}>
          Quelle: {meta.source}. Stand der Daten: {new Date(meta.updated).toLocaleDateString('de-DE')}.
          NDVI = (NIR − Rot) / (NIR + Rot); Wolken über die Scene Classification maskiert,
          Wolkenlücke relativ zur Landkreisfläche. Prototyp — eine Testregion.
        </p>
      </div>

      <style>{`
        @media (max-width: 840px) { .veg-grid { grid-template-columns: 1fr !important; } }
      `}</style>
    </>
  )
}
...
function Legend2() {
  ...
  return (
    <div style={{ display: 'flex', gap: 15, fontSize: 12, color: '#64748b', marginTop: 9, flexWrap: 'wrap' }}>
      {item('#16a34a', 'Gewähltes Jahr')}
      {item('#94a3b8', 'Mittel')}
      {item('#e2e8f0', 'Spannweite')}
    </div>
  )
}
